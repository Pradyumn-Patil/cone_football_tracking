...
import numpy as np
from cone_tracker_enhanced import EnhancedConeTracker
...
def extract_yolo_features(model, frame, detections):
    """
    Extract intermediate features from YOLO model for appearance tracking
    This is a placeholder - full implementation would hook into model layers
    """
    # For now, return None - in full implementation, this would:
    # 1. Hook into YOLO backbone layers
    # 2. Extract feature maps
    # 3. Crop features for each detection
...
def process_video_enhanced(video_path, model_path=None, output_path=None, conf_threshold=0.5, 
                          show_preview=False, imgsz=None, track_cones=True, show_trails=False, 
                          save_tracks=None, strict_validation=True, football_buffer=50,
                          use_enhanced_tracker=True, max_cones=None, learn_layout=False):
    """
    Process video with enhanced tracking and re-identification
    """
    ...
    if output_path is None:
        ...
        suffix = "_enhanced" if use_enhanced_tracker else "_detected"
    ...
    # Initialize enhanced cone tracker
    cone_tracker = None
    cone_trails = {}  # Store trails for each cone
    if track_cones and use_enhanced_tracker:
        cone_tracker = EnhancedConeTracker(
            track_activation_threshold=0.5,
            high_match_threshold=0.8,
            low_match_threshold=0.5,
            max_time_lost=60,  # Keep tracks longer
            min_hits=3,
        ...
        print(f"Enhanced cone tracking enabled (strict validation: {strict_validation}, football buffer: {football_buffer}px)")
        print("Using ByteTrack-style two-stage association with re-identification")
        if max_cones:
            print(f"Cone count limited to: {max_cones}")
        if learn_layout:
            print("Spatial layout learning enabled")
    ...
    try:
        while True:
                    ...
                    for box in result.boxes:
                        if hasattr(box, 'xyxy') and len(box.xyxy) > 0:
                            ...
                            if is_football:
                                ...
                                cone_detections.append(detection_info)
                            else:
            ...
            if track_cones and cone_tracker:
                # Extract features if using enhanced tracker
                appearance_features = None
                if use_enhanced_tracker:
                    appearance_features = extract_yolo_features(model, frame, cone_detections)
                
                # Update tracker with frame for color histograms
                tracked_cones = cone_tracker.update(cone_detections, frame=frame, 
            ...
            for cone in tracked_cones:
                ...
                state = cone.get('state', 'confirmed')
                ...
                status_label = f"{state}: {confidence:.2f}"
                ...
                status_size, _ = cv2.getTextSize(status_label, cv2.FONT_HERSHEY_SIMPLEX, 0.5, 1)
                ...
                # Draw status below ID (smaller)
                cv2.rectangle(frame, 
                            (x1, y1 - label_size[1] - status_size[1] - 12),
                            (x1 + status_size[0] + 4, y1 - label_size[1] - 8),
                            (0, 0, 0), -1)
            ...
            if track_cones and cone_tracker:
                ...
                text = f"Cones: {summary['confirmed_tracks']} confirmed, {summary['tentative_tracks']} tentative"
                if use_enhanced_tracker:
                    text += f", {summary['lost_buffer_tracks']} in re-ID buffer"
            ...
            if show_preview and frame_count % 2 == 0:
                ...
                cv2.imshow('Enhanced Object Detection', preview_frame)
...
def main():
    parser = argparse.ArgumentParser(description='Enhanced object detection with re-identification')
    ...
    # Tracking options
    parser.add_argument('--track-cones', action='store_true', default=True,
                       help='Enable cone tracking with unique IDs (default: True)')
    ...
    parser.add_argument('--use-enhanced', action='store_true', default=True,
                       help='Use enhanced tracker with re-identification (default: True)')
    parser.add_argument('--max-cones', type=int, default=None,
                       help='Maximum number of cones in the video (helps prevent ID switches)')
    parser.add_argument('--learn-layout', action='store_true',
                       help='Learn spatial layout of cones for better re-identification')
    ...
    success = process_video_enhanced(
        ...
        use_enhanced_tracker=args.use_enhanced,
        max_cones=args.max_cones,
        learn_layout=args.learn_layout
    )
